...
exports.json = function(req, res, files, next, dir, showUp, icons, path){
  ...
  var prefix = url.resolve((req.connection.encrypted ? 'https': 'http') + '://' + req.headers.host, originalDir);
  ...
  stat(path, files, function(err, stats){
    ...
    files = files.map(function(file, i){
      if(stats[i].isDirectory())
        return { type: "dir", class: "directory collapsed", path: url.resolve(prefix,file)+"/", text: file };
      else{
        var e = require('path').extname(file).substring(1);
        return { type: "file", class: "file ext_"+e, path: url.resolve(prefix,file), text: file };
      }
    }).filter(function(f){return f.type==="dir" || f.class.indexOf("xml", f.class.length - 3) !== -1;});
